...
public class RecordVectorWindXTC {
	...
	lcrc = crc_chk_seeded(lCRC, current.gnss_buff, current.gnss_length);
		...
		for ( i=0 ; i<n.length ; i++ ) {
			...
			/* add our PSAT parser */
			sf.registerParser("SAT", NMEASentenceParserPSAT.class);

			if ( debug ) {
				System.err.println("# SentenceFactory has following parsers: " + sf.listParsers().toString() );
			}
			...
			try {
				...
				System.err.println("# sentence '" + sentence + "' is unsupported.");
			}
